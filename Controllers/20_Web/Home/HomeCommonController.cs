...
namespace Gemini.Controllers._20_Web.Home
{
    public class HomeCommonController : GeminiController
    {
            ...
            if (!string.IsNullOrEmpty(seoFriendUrl))
            {
                ...
                if (models.PosProduce != null)
                {
                    ...
                    models.ListProduceSameCreatedBy = (from pp in DataGemini.PosProduces
                                                       join pc in DataGemini.PosCategories on pp.GuidCategory equals pc.Guid
                                                       where pp.Active && pc.Active && pp.ApprovedStatus == PosProduce_ApprovedStatus.Approved && pp.CreatedBy == models.PosProduce.CreatedBy && pp.Guid != models.PosProduce.Guid && pp.HotProduce == true
                                                       select new PosProduceModel
                                                       {
                                                           Guid = pp.Guid,
                                                           Name = pp.Name,
                    ...
                    models.ListProduceSameCategory = (from pp in DataGemini.PosProduces
                                                      join pc in DataGemini.PosCategories on pp.GuidCategory equals pc.Guid
                                                      where pp.Active && pc.Active && pp.ApprovedStatus == PosProduce_ApprovedStatus.Approved && pp.GuidCategory == models.PosProduce.GuidCategory && pp.Guid != models.PosProduce.Guid && pp.TopProduce == true
                                                      select new PosProduceModel
                                                      {
                                                          Guid = pp.Guid,
                                                          Name = pp.Name,
                                                          ...
                                                          //Legit = pp.Legit,
                                                          //LegitCount = pp.LegitCount
                                                      //}).OrderByDescending(s => s.Price).Take(4).ToList();
                                                      }).Take(4).ToList();
        ...
        {
            var username = GetUserInSession();
            var userInfo = DataGemini.SUsers.Where(x=>x.Username == username).FirstOrDefault();
            ViewBag.CurrentUsername = username;
            ViewBag.UserPhone = userInfo.Mobile;
